...
**Home Assistant stores the default dashboard per user, and the API applies it
to whoever the connection authenticated as — always this agent's own account.**
There is no way to set it for anyone else.

So when the owner asks you to change "my home screen", the honest answer is that
you can only change your own account's, and theirs has to be set by them, in
their own Home Assistant profile (Settings → click your user → Dashboard). Do
not silently change your own and report it as done — that would look like
success and change nothing they can see.
